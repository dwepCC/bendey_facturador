...
class InstancesConfig 
{
    ...
    public function __construct(array $config = [])
    {
        ...
        if (array_key_exists('handlers', $config)) {
            $this->_usedProperties['handlers'] = true;
            $this->handlers = new \Symfony\Config\JmsSerializer\InstancesConfig\HandlersConfig($config['handlers']);
            unset($config['handlers']);
        }

        if (array_key_exists('subscribers', $config)) {
            $this->_usedProperties['subscribers'] = true;
            $this->subscribers = new \Symfony\Config\JmsSerializer\InstancesConfig\SubscribersConfig($config['subscribers']);
            unset($config['subscribers']);
        }

        if (array_key_exists('object_constructors', $config)) {
            $this->_usedProperties['objectConstructors'] = true;
            $this->objectConstructors = new \Symfony\Config\JmsSerializer\InstancesConfig\ObjectConstructorsConfig($config['object_constructors']);
            unset($config['object_constructors']);
        }

        if (array_key_exists('property_naming', $config)) {
            $this->_usedProperties['propertyNaming'] = true;
            $this->propertyNaming = \is_array($config['property_naming']) ? new \Symfony\Config\JmsSerializer\InstancesConfig\PropertyNamingConfig($config['property_naming']) : $config['property_naming'];
            unset($config['property_naming']);
        }

        if (array_key_exists('expression_evaluator', $config)) {
            $this->_usedProperties['expressionEvaluator'] = true;
            $this->expressionEvaluator = \is_array($config['expression_evaluator']) ? new \Symfony\Config\JmsSerializer\InstancesConfig\ExpressionEvaluatorConfig($config['expression_evaluator']) : $config['expression_evaluator'];
            unset($config['expression_evaluator']);
        }

        if (array_key_exists('metadata', $config)) {
            $this->_usedProperties['metadata'] = true;
            $this->metadata = new \Symfony\Config\JmsSerializer\InstancesConfig\MetadataConfig($config['metadata']);
            unset($config['metadata']);
        }

        if (array_key_exists('visitors', $config)) {
            $this->_usedProperties['visitors'] = true;
            $this->visitors = new \Symfony\Config\JmsSerializer\InstancesConfig\VisitorsConfig($config['visitors']);
            unset($config['visitors']);
        }

        if (array_key_exists('default_context', $config)) {
            $this->_usedProperties['defaultContext'] = true;
            $this->defaultContext = new \Symfony\Config\JmsSerializer\InstancesConfig\DefaultContextConfig($config['default_context']);
